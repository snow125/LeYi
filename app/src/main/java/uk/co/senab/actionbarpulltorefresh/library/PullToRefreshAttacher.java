...
public class PullToRefreshAttacher implements View.OnTouchListener {
    ...
    public PullToRefreshAttacher(Activity activity, Options options) {
        ...
        mHeaderContent = mHeaderView.findViewById(R.id.ptr_content);
    ...
    /**
     * Set the view which will be used to initiate refresh requests and a
     * listener to be invoked when a refresh is started. This version of the
     * method will try to find a handler for the view from the built-in view
     * delegates.
     * 
     * @param view - View which will be used to initiate refresh requests.
     * @param refreshListener - Listener to be invoked when a refresh is
     *            started.
     */
    ...
    /**
     * Set the view which will be used to initiate refresh requests, along with
     * a delegate which knows how to handle the given view, and a listener to be
     * invoked when a refresh is started.
     * 
     * @param view - View which will be used to initiate refresh requests.
     * @param viewDelegate - delegate which knows how to handle
     *            <code>view</code>.
     * @param refreshListener - Listener to be invoked when a refresh is
     *            started.
     */
     ...
     * @param refreshListener - Listener to be invoked when a refresh is
     *            started.
     * @param alterTouchListeners - Whether to set this as the touch listener
     *            for <code>view</code>. You should only provide
     *            <code>false</code> here if you will propagate the touch events
     *            to this object yourself.
     */
    public void setRefreshableView(View view, ViewDelegate viewDelegate,
    ...
    void onPullStarted(float y) {
        ...
        if (mHeaderInAnimation != null) {
            mHeaderContent.startAnimation(mHeaderInAnimation);
        }
	...
	private void reset(boolean fromTouch) {
        ...
        if (mHeaderView.getVisibility() != View.GONE) {
            // Hide Header
            if (mHeaderOutAnimation != null) {
                mHeaderContent.startAnimation(mHeaderOutAnimation);
                ObjectAnimator.ofInt(mHeaderProgressBar, "Progress", 0)
    ...
    public static class DefaultHeaderTransformer extends HeaderTransformer {
        ...
        public void onViewCreated(Activity activity, View headerView) {
            ...
            mContentView = headerView.findViewById(R.id.ptr_content);
            if (mContentView != null) {
                mContentView.getLayoutParams().height = getActionBarSize(activity);
